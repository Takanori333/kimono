    ...
    function sendMsg(){
        if(msg.value){
            socket.emit('stylist_send',{stylist_id:stylist_id,customer_id:customer_id,message:msg.value});
            $.ajax({
                url:url,
                type:'POST',
                data:{"_token":csrf,stylist_id:stylist_id,customer_id:customer_id,message:msg.value,from:0},
                success:function(){
                    console.log(1);
                },
                error:function(msg){
