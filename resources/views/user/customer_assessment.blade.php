...
<body>
    ...
    <div class="container">
        <div class="contents pt-5 mt-5 w-75 mx-auto">
            ...
            <!-- 評価 -->
            <div class="my-4">
                <p class="d-inline me-3">購入者評価</p>
                <div class="Stars" style="--rating: {{ round($page_user->customer_assessment->avg('point'), 1) }};" aria-label="Rating of this product is 3.5 out of 5.">
                    @if (round($page_user->customer_assessment->avg("point"), 1) == 0)
                    <p class="d-inline">-</p>
                    @else
                    <p class="d-inline">{{ round($page_user->customer_assessment->avg("point"), 1) }}</p>
                    @endif
                </div>
            </div>
            <!-- </div> -->
    ...
    <br>
    評価{{ round($page_user->customer_assessment->avg("point"), 1) }}</a>
    <div class="Stars" id="star" style="--rating: {{ round($page_user->customer_assessment->avg("point"), 1) }};"></div>
    <h2>評価一覧</h2>
    {{-- 評価詳細 --}}
    @if ($assessment_users->isNotEmpty())
        @foreach ($assessment_users as $assessment_user)
